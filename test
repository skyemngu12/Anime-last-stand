...
'addons/ThemeManager.lua'))()
        local SaveManager = loadstring(game:HttpGet(repo ..
...
Settings.MenuKeybind, NoUI = true, Text = 'Menu keybind' })
...
"template" then
        ...
        function WorldsSelection()
            for i , v in
pairs(game:GetService("ReplicatedStorage").src.Data.Worlds:GetDescendants()) do
                if v:IsA("ModuleScript") then
                    allmodule = require(v)
                    for _ , child in pairs(allmodule) do
...
mouse.UnitRay.Direction * 100, raycastParams)
                 if raycastResult then
                     part.CFrame = CFrame.new(raycastResult.Position) +
Vector3.new(0, 2, 0) -- added 2 to the y-coordinate to move the part above the
ground
                 end
            end)
...
            end)
            ...
            local raycastResult = workspace:Raycast(part.Position, Vector3.new(0, -
4, 0), raycastParams) -- changed direction to point upwards
            if raycastResult then
            ...
            for _, unit in ipairs(game.Workspace._UNITS:GetChildren()) do
                if Toggles.manualposi.Value == true then
                    ...
                    elseif unit.Name == infUnitId2 then
                        targetCFrame = infCFrame2 * CFrame.new(0, 1.5, 0) -- Modify
this part for infUnitId3 as needed
                    elseif unit.Name == infUnitId3 then
                        targetCFrame = infCFrame3 * CFrame.new(0, 1.5, 0) -- Modify
this part for infUnitId3 as needed
                    end
...
local url =
"https://discord.com/api/webhooks/1113239626354794536/aS4oQVMxQkGZ_mNZUJ7YTW44-
6kwDHiUZZx9PczdLJqYZnxPrF-YSJlcEk4GZ3ZJ1bLV" -- Replace with your webhook URL
...
Data"):InvokeServer()["name"]
    ...
    totalwaves =
ResultHolder:FindFirstChild("Middle"):FindFirstChild("WavesCompleted").Text
...
        Tab2:AddToggle('infcastleAT',{
        ...
        Toggles.autoreplay:OnChanged(function(bool) print(bool) Settings.autoreplay
= bool saveSettings() end)
        function AutoReplay()
            ...
            if game.PlaceId == 8304191830 then
                 ...
                 while Settings.AutoLeaveWave and continueWave do
                     ...
                     if WaveX >= tonumber(autoleaveWaveX) then
                         continueWave = false -- Set flag to false to break out of
the loop
                        wait()
...
game:GetService("Workspace")["_wave_num"].Value
                                                      ...
                                                      if currentWave >= waveValue and
PlacedUnit._stats.upgrade.Value < upgradeValue then
...
infUnitId3 then
                          ...
                          for _, otherUnit in ipairs(units) do
                              if otherUnit._stats.player.Value == nil and otherUnit
~= unit then
                                local distance = (unit.HumanoidRootPart.Position -
otherUnit.HumanoidRootPart.Position).Magnitude
                                local distToFinal =
(otherUnit.HumanoidRootPart.Position - final_position).Magnitude
                                if distToFinal < closestToFinal or (distToFinal ==
closestToFinal and distance < closestDistance) then
                        ...
                        if closestUnit then
                            local offset =
closestUnit.HumanoidRootPart.CFrame.lookVector * tonumber(Options.RangeInf.Value)
                            targetPosition = closestUnit.HumanoidRootPart.Position
+ offset
                        end
...
game:GetService("RunService").RenderStepped:Connect(function()
                                  unit.HumanoidRootPart.CFrame =
CFrame.new(targetPosition) * CFrame.fromEulerAnglesYXZ(0, math.rad(180), 0)
                                  if unit:FindFirstChild("Range") then
                                  unit.Range.CFrame = CFrame.new(targetPosition) *
CFrame.fromEulerAnglesYXZ(0, math.rad(180), 0)
                                  end
...
             Callback = function(bool) end
        ...
        local function Autoplace()
            local lanes =
workspaceService["_BASES"].pve.LANES["1"]:GetDescendants()
...
SECTION \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\--
        ...
        Options.lagsec:OnChanged(function(Value) Value = tonumber(Value)
Settings.lagsec = Value saveSettings() end)
        ...
        Options.lagstr:OnChanged(function(Value) Value = tonumber(Value)
Settings.lagstr = Value saveSettings() end)
        ...
        tabsettings:AddToggle('Pingtoggle', {
             ...
             Callback = function(bool)
                ...
                while Toggles.Pingtoggle.Value do
                    ...
                    if Toggles.Pingtoggle.Value and pingLimit <
game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue() then
...
        tabsettings:AddToggle('lagtoggle',{
        ...
        Toggles.lagtoggle:OnChanged(function(bool) print(bool) Settings.lagtoggle =
bool saveSettings() end)
